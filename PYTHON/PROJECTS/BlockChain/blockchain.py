...
# OrderedDict is a class from collections that preserves the order of keys as they were added
from collections import OrderedDict
from hash_util import hash_block, hash_string_256
import json

MINING_REWARD = 10  # Reward to Miners for mining blocks
blockchain = []
...
def load_data():
    ...
    try:
        with open('./secure_blockchain.txt', 'r') as f:
            # alternatively can also use pickling data using pickel library to create portable serialized objects.
            file_content = f.readlines()
            blockchain = json.loads(file_content[0][:-1])
            updated_blockchain = []
            for block in blockchain:
                updated_block = {'previous_hash': block['previous_hash'], 'index': block['index'], 'proof': block['proof'], 'transactions': [
        ...
        genesis_block = {'previous_hash': '', 'index': 0,
                         'transactions': [], 'proof': 100}  # The very first block
...
def add_transaction(recipient, sender=owner, amount=1.0):
    ...
    # using dictionary to store the transaction details
...
def mine_block():
    ...
    block = {'previous_hash': hashed_block, 'index': len(
        blockchain), 'transactions': copied_transaction, 'proof': proof}
